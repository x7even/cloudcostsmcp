...
TEST_PROMPTS = {
    ...
    "CL2": (
        ...
        "Tiers:\n"
        "- Web tier: 2 × m5.large EC2 (Linux, on-demand); ALB — include the"
        " ALB hourly charge and 10 LCU × 730 hours/month.\n"
        "- Database tier: RDS db.m5.large MySQL 8.0 Multi-AZ in us-east-1, 200"
        " GB gp3 storage, 3 000 provisioned IOPS.\n"
        "- Object storage: S3 Standard — 500 GB/month storage cost only (S3"
        " per-request pricing is not required; if it is absent from the catalog,"
        " note it and skip).\n"
        "- Backup: RDS automated snapshot storage at 200 GB"
        " (storage_type=\"rds-snapshot\"); if a Glacier archival rate is not"
        " in the catalog on the first lookup attempt, note it as unavailable"
        " and exclude it from the total.\n\n"
        "Rule: if any line item is not found after a single describe_catalog"
        " or get_price attempt, mark it as unavailable and move on — do not"
        " re-search with alternative terms. Use estimate_bom to aggregate the"
        " tiers for which pricing was found into a single monthly total."
    ),
    # --- Cluster / Workload ---
    "XK1": (
