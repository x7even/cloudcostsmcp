...
async def test_cache_stats_reflects_inserted_prices(
    availability_tools, cache_manager: CacheManager
):
...
async def test_refresh_cache_does_not_delete_other_provider(
    lookup_tools, cache_manager: CacheManager
):
    """refresh_cache(provider='aws') must leave gcp entries untouched."""
    await cache_manager.set_prices("aws", "compute", "us-east-1", {}, [{"p": "1"}], ttl_hours=1)
    await cache_manager.set_prices("gcp", "compute", "us-central1", {}, [{"p": "2"}], ttl_hours=1)
