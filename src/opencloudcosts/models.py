...
class NormalizedPrice(BaseModel):
    ...
    cache_age_seconds: int | None = None  # seconds since fetched_at (set on cache hit)
    ...
    def summary(self) -> dict[str, Any]:
        ...
        if self.fetched_at is not None:
            result["as_of"] = self.fetched_at.isoformat()
...
class ComputePricingSpec(BasePricingSpec):
    ...
    def cache_key(self) -> str:
        ...
        return (
            f"{base}:{self.resource_type or ''}:{self.os}:{self.vcpu or ''}:{self.memory_gb or ''}"
        )
